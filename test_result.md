...
## agent_communication:
    -agent: "main"
    -message: "🎨 UI COLOR UNIFORMIZATION + PROSPECT API FIXES COMPLETED: 1) ✅ EmployeeDashboard color refinement - replaced all green theme elements with blue/slate to match dark theme (stat cards border-l-blue-500, step indicators bg-blue-500, visitor badges bg-blue-500/20, dialog backgrounds bg-[#1E293B]). 2) ✅ ConsultantDashboard API prefix fix - corrected all API calls to use /api prefix (/api/contact-messages, /api/contact-messages/{id}/consultant-notes, /api/contact-messages/{id}/convert-to-client) to fix 403 errors. Backend already properly filters prospects by status='paiement_50k' for CONSULTANT role. 3) 🔍 NEXT: Verify prospect list display across all roles (SuperAdmin should see ALL, Manager/Employee see ASSIGNED, Consultant sees PAID prospects with status paiement_50k). Ready for backend + frontend testing."
    -agent: "testing"
